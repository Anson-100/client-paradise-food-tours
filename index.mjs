...
import {
  fitnessClauses,
  martialArtsClauses,
} from "/opt/nodejs/waiverClauses.js"
...
export const handler = async event => {
  try {
    ...
    const waiverId = `${Date.now().toString().slice(-6)}`
    ...
    const waiverItem = {
      ...
      relationshipToChild: requestBody.relationshipToChild || null,
      otherRelationship: requestBody.otherRelationship || null,
      waiverData: {},
      clauses:
        requestBody.waiverType === "martial_arts"
          ? martialArtsClauses
          : fitnessClauses,
    }
    ...
    const dynamoItem = {
      ...
      relationshipToChild: waiverItem.relationshipToChild,
      otherRelationship: waiverItem.otherRelationship,
    }
    ...
    await uploadToS3(
      ...
      requestBody.email,
      requestBody.signingForChild,
      requestBody.childFirstName,
      requestBody.childLastName
    )
...
const generatePDF = async waiverData => {
  return new Promise((resolve, reject) => {
    ...
    doc
      .font("Helvetica")
      .text(
        `${waiverData.emergencyContactName} (${waiverData.emergencyContactPhone})`
      )
    ...
    if (waiverData.signingForChild && waiverData.child) {
      doc
        .font("Helvetica-Bold")
        .text("Relationship to Child: ", { continued: true })
      const relationshipText =
        waiverData.relationshipToChild === "Other"
          ? waiverData.otherRelationship
          : waiverData.relationshipToChild
      doc.font("Helvetica").text(relationshipText)
      doc.moveDown()
      doc.moveDown()
      ...
      doc
        .font("Helvetica")
        .text(`${waiverData.child.firstName} ${waiverData.child.lastName}`)

      doc
        .font("Helvetica-Bold")
        .text("Child Date of Birth: ", { continued: true })
    ...
    doc
      .font("Helvetica")
      .text("I agree to the terms and conditions of this liability waiver")
    doc.moveDown()
...
const uploadToS3 = async (
  ...
  const name =
    signingForChild && childFirstName && childLastName
      ? `${childLastName.toLowerCase()}_${childFirstName.toLowerCase()}_CHILD`
      : `${lastName.toLowerCase()}_${firstName.toLowerCase()}`

  const fileName = `${name}_${waiverType === "martial_arts" ? "MA" : "F"}_${waiverId}.pdf`
  ...
  const params = {
    ...
    Metadata: {
      email: email, // ✅ Store the email in metadata
      firstname: firstName,
    },
  }

  console.log(`Uploading PDF to S3: ${fileName} with email metadata: ${email}`)

  await s3.send(new PutObjectCommand(params))
  return fileName // ✅ Return filename so it can be used elsewhere
}
